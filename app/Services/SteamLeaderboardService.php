...
class SteamLeaderboardService
{
    ...
    /**
     * Get all user rankings across all leaderboards (asynchronous)
     */
    public function getAllUserRankings(string $steamId): array
    {
        ...
        // Prepare async requests for all leaderboards
        $requestsToMake = [];
        $leaderboardMap = [];
        ...
        foreach ($leaderboards as $index => $leaderboard) {
            if (isset($leaderboard['id'])) {
                ...
                if ($cachedRank !== null) {
                    if ($cachedRank) {
                        $leaderboards[$index]['rank_data'] = $cachedRank;
                    }
                    continue;
                }

                // If not in cache, prepare async request parameters
                if ($this->publisherApiKey) {
                    $requestsToMake["leaderboard_{$index}"] = [
                        'key' => $this->publisherApiKey,
                        'appid' => $this->appId,
                        'rangestart' => 0,
                        'rangeend' => 25,
                    ...
                    $leaderboardMap["leaderboard_{$index}"] = [
                        'index' => $index,
        ...
        if (!empty($requestsToMake)) {
            $responses = Http::pool(function ($pool) use ($requestsToMake) {
                foreach ($requestsToMake as $key => $params) {
                    $pool->as($key)->timeout(10)->get('https://partner.steam-api.com/ISteamLeaderboards/GetLeaderboardEntries/v1/', $params);
                }
            });

            // Process responses
            foreach ($responses as $key => $response) {
                $mapData = $leaderboardMap[$key];
                $index = $mapData['index'];
                $leaderboard = $mapData['leaderboard'];
                $cacheKey = $mapData['cache_key'];
                ...
                if ($response->successful()) {
                    ...
                    Log::warning('Failed to fetch Steam user rank in batch', [
                ...
                if ($rankData) {
                    $leaderboards[$index]['rank_data'] = $rankData;
                }
            }
        }

        return array_values($leaderboards);
    }
